...
embeddings = torch.load("savedEmbeddings.pt")
...
top5 = [counter[0], counter[5], counter[8], counter[7], counter[11], counter[12]]
tagmaplist = [tagmap[x] for x in top5]
print(top5, tagmaplist)
idxlist = []
for i in range(6):
    idxlist.append(np.where(img_tags == top5[i]))
embedlist = tuple(img_embed[idxlist[i]] for i in range(6))
embedlist_aud = tuple(aud_embed[idxlist[i]] for i in range(6))
